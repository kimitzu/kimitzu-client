...
export default class OpenLocationCode {
  ...
  public static readonly LATITUDE_MAX_ = LATITUDE_MAX

  // The maximum value for longitude in degrees.
  public static readonly LONGITUDE_MAX_ = LONGITUDE_MAX
  ...
  public static isValid(code: string): boolean {
    ...
    if (
      code.indexOf(OpenLocationCode.SEPARATOR_) !== code.lastIndexOf(OpenLocationCode.SEPARATOR_)
    ) {
      return false
    }
  ...
  public static encode(
    ...
    let code = OpenLocationCode.encodePairs(
      clippedLatitude,
      clippedLongitude,
      Math.min(codeLength, OpenLocationCode.PAIR_CODE_LENGTH_)
    )
    // If the requested length indicates we want grid refined codes.
    if (codeLength > OpenLocationCode.PAIR_CODE_LENGTH_) {
      code += OpenLocationCode.encodeGrid(
        clippedLatitude,
        clippedLongitude,
  ...
  public static decode(code: string): CodeArea {
    ...
    const codeArea = OpenLocationCode.decodePairs(
      editedCode.substring(0, OpenLocationCode.PAIR_CODE_LENGTH_)
    ...
    const gridArea = OpenLocationCode.decodeGrid(
      editedCode.substring(OpenLocationCode.PAIR_CODE_LENGTH_, OpenLocationCode.MAX_DIGIT_COUNT)
    )
    return new CodeArea(
  ...
  public static recoverNearest(shortCode: string, latitude: number, longitude: number): string {
    ...
    const codeArea = OpenLocationCode.decode(
      OpenLocationCode.encode(referenceLatitude, referenceLongitude).substr(0, paddingLength) +
        shortCodeUpper
    )
  ...
  public static shorten(code: string, latitude: number, longitude: number): string {
    ...
    if (codeArea.codeLength < OpenLocationCode.MIN_TRIMMABLE_CODE_LEN_) {
      throw new Error(
        'ValueError: Code length must be at least ' + OpenLocationCode.MIN_TRIMMABLE_CODE_LEN_
      )
    }
  ...
  private static encodePairs(latitude: number, longitude: number, codeLength: number): string {
    ...
    if (code.length < OpenLocationCode.SEPARATOR_POSITION_) {
      code =
        code +
        Array(OpenLocationCode.SEPARATOR_POSITION_ - code.length + 1).join(
  ...
  private static decodePairsSequence(code: string, offset: number): number[] {
    ...
    while (i * 2 + offset < code.length) {
      value +=
        OpenLocationCode.CODE_ALPHABET_.indexOf(code.charAt(i * 2 + offset)) *
        OpenLocationCode.PAIR_RESOLUTIONS_[i]
      i += 1
    }
